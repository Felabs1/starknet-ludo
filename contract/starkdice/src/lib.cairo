...
mod StarkDice {
    ...
    use super::{
        OptionTrait, ArrayTrait, Player, Game, Reward, GameReward, get_caller_address,
        ContractAddress
    };
    #[storage]
    struct Storage {
    ...
    impl IStarkDiceImpl of super::IStarkDice<ContractState> {
        fn register_player(ref self: ContractState, username: felt252) {
            ...
            let player: Player = Player {
                id: player_id, username: username, games_played: 0, rewards_count: 0,
            };
        ...
        fn get_game_player(self: @ContractState, game_id: u256, me: ContractAddress) -> u32 {
            ...
            let out = game.player1;
            if game.player1 == Option::Some(me) {
                player_number = 1;
            } else if game.player2 == Option::Some(me) {
                player_number = 2;
            } else if game.player3 == Option::Some(me) {
                player_number = 3;
            } else if game.player4 == Option::Some(me) {
                player_number = 4;
            }
